...
def _runtime_config(state: dict[str, Any], avatar_path: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "messages": {
            ...
            "reserved_current": str(content.get("reservedCurrent") or "--"),
            "reserved_level_label": str(
                content.get("reservedLevelLabel") or "好感度等级"
            ),
            "reserved_level_text": str(content.get("reservedLevelText") or "未接入"),
            "reserved_attitude": str(
                content.get("reservedAttitude") or "对你的态度: 未接入"
            ),
            "reserved_next_text": str(
                content.get("reservedNextText") or "距离升级还差--好感度"
            ),
        ...
        "reserved_panel": {
            ...
            "current": str(content.get("reservedCurrent") or "--"),
            "level_label": str(content.get("reservedLevelLabel") or "好感度等级"),
            "level_text": str(content.get("reservedLevelText") or "未接入"),
            "attitude": str(content.get("reservedAttitude") or "对你的态度: 未接入"),
            "next_text": str(content.get("reservedNextText") or "距离升级还差--好感度"),
            "separator": str(content.get("reservedSeparator") or ": "),
            "progress": _as_float(content.get("reservedProgress"), 0, 0, 100),
            "heart_count": int(_as_float(content.get("heartCount"), 8, 0, 16)),
...
def _generate_bundle(
    ...
    (output_dir / "README.md").write_text(
        ...
        "ZIP 无需解压。\n"
        "4. 发送 `签到模板` 确认已发现；设置页默认会自动应用新模板。\n"
        f"5. 如需手动切换，管理员发送 `切换签到模板 {pack_name}`，再发送 `签到` 验证。\n"
        "6. 若存在同名模板，请根据 `签到模板` 列表发送 "
        "`切换签到模板 #序号`。\n\n"
        "## 说明\n\n"
        "- 内部随机 ID 只用于唯一识别和保存选择状态，不作为显示名称。\n"
