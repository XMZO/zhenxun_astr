...
async function generateTemplate() {
    ...
    try {
        ...
        $("#generateResult").innerHTML = `<p>模板 <strong>${escapeHtml(result.pack_name)}</strong> 已自动安装到插件的 <code>template_packs</code> 目录。</p><p>在 AstrBot 中重载“真寻签到”，打开插件配置，从“当前模板”下拉框选择 <strong>${escapeHtml(result.pack_name)}</strong> 并保存。</p><p>迁移到其他实例：下载 ZIP，在“插件配置 -> 签到模板管理”中上传并保存。</p><p class="warning-line">内部随机 ID 仅用于唯一识别，不会显示在设置下拉框；输出副本保存在 ${escapeHtml(result.directory)}。</p><a href="${escapeHtml(result.download)}" download>下载模板 ZIP</a>`;
