...
- 默认分别缓存每位用户当天的“今日签到”和“我的信息”图片；相同状态会直接复用，次日自动清理。
- 图片渲染失败时回退为文字回复。
...
“每日图片缓存”默认开启，缓存目录为 `data/plugin_data/astrbot_plugin_zhenxun_sign/card_cache/<日期>`。同一用户、同一模板、同一卡片模式且签到数据未变化时会复用图片；“今日签到”和“我的信息”分别缓存，不会互相命中。真正签到导致次数或奖励变化时会生成新图，切换模板也会建立独立缓存。启用好感度联动后，好感度、关系、等级、进度或作用域变化也会产生新的缓存键，因此下一次签到或查看会立即生成更新后的卡片。日期变化时自动删除旧日期目录。关闭该设置后，每条签到或查看指令都会重新生成图片；需要立即删除已有成图时，可勾选“清空现有缓存”并保存，该选项执行后会自动关闭，签到记录、模板和头像缓存不会被删除。
