...
def test_default_editor_state_renders_without_unresolved_template_fields() -> None:
    ...
    assert "font-family: 'kcytFont', 'shFont', sans-serif;" in rendered
...
def test_runtime_config_does_not_include_editor_only_theme() -> None:
    ...
    assert config["reserved_panel"]["current"] == "{favour}"
    assert config["reserved_panel"]["level_text"] == "{favour_level}"
    assert config["reserved_panel"]["max_text"] == "已达到最高好感等级"
...
def test_editor_surface_keeps_selection_overlay_and_launcher() -> None:
    ...
    assert 'data-bind="content.reservedMaxText"' in index
    assert "{favour_relationship}" in index
...
def test_generator_installs_self_contained_folder_and_zip(tmp_path) -> None:
    ...
    assert folder_pack.settings["reserved_panel"]["current"] == "{favour}"
    ...
    assert "font-family: 'kcytFont', 'shFont', sans-serif;" in folder_pack.read_text(
        "sign_card.css"
    )
