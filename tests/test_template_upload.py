...
def test_settings_upload_installs_and_activates_template(tmp_path: Path) -> None:
    ...
    packs, _ = plugin._discover_templates()
    plugin._sync_template_schema(packs)
    active_field = plugin.config.schema["template_management"]["items"][
        "active_template"
    ]
    assert "tpl_settings_upload" in active_field["options"]
    assert "设置页上传模板" in active_field["labels"]

    plugin._set_selected_template_id("default")
    plugin._import_configured_template_packages()
...
def test_schema_exposes_native_zip_uploader() -> None:
    ...
    assert management["items"]["active_template"]["options"] == [
        "default",
        "tpl_fb516978e6e841458199c80ad1f6bcc5",
    ]
    assert management["items"]["active_template"]["labels"] == [
        "官方默认模板",
        "夏娜",
    ]
    assert schema["command_prefix_mode"]["options"] == [
        "slash",
        "none",
        "hash",
        "custom",
    ]
    assert schema["render_backend"]["options"] == ["auto", "local", "remote"]
    assert schema["render_backend"]["default"] == "auto"
    assert schema["image_cache"]["items"]["enabled"]["default"] is True
    assert schema["custom_command_prefix"]["condition"] == {
        "command_prefix_mode": "custom"
    }
