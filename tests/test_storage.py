...
async def test_sign_can_cross_year_boundary(tmp_path):
    ...
    assert next_year_record["uid"] == 1


@pytest.mark.asyncio
async def test_internal_uids_are_sequential_and_persistent(tmp_path):
    ...
    second = await store.sign(
        key="qq::10002",
        user_id="10002",
        platform="qq",
        display_name="第二个用户",
        today="2026-08-11",
        signed_at="2026-08-11T00:01:00+08:00",
    )

    assert first["uid"] == 1
    assert second[0]["uid"] == 2

    reloaded = SignStore(data_path)
    assert (
        await reloaded.get_record(
            key="qq::10001",
            user_id="10001",
            platform="qq",
            display_name="第一个用户",
